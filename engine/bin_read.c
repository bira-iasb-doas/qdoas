...
void cal_options_GADS_getbin (FILE* unit, CAL_OPTIONS_GADS *var)
 {
  ...
  fread(var,sizeof(CAL_OPTIONS_GADS),1,unit);
   ...
   swap_bytes_float((unsigned char *)&var->start_lat);
   swap_bytes_float((unsigned char *)&var->start_lon);
   swap_bytes_float((unsigned char *)&var->end_lat);
   swap_bytes_float((unsigned char *)&var->end_lon);
