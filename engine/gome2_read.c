...
typedef struct _GOME2OrbitFiles                                                 // description of an orbit
 {
 	unsigned char               gome2FileName[MAX_STR_LEN+1];                             // the name of the file with a part of the orbit
...
RC Gome2BrowseMDR(GOME2_ORBIT_FILE *pOrbitFile,INDEX indexBand)
 {
  ...
  unsigned char geoFileName[MAX_STR_SHORT_LEN+1];
  FILE *geoFp;
  INDEX i;
  RC  rc;
...
RC GOME2_Set(ENGINE_CONTEXT *pEngineContext)
 {
  ...
  unsigned char fileFilter[MAX_STR_SHORT_LEN+1],fileFilterSub[MAX_STR_SHORT_LEN+1];
