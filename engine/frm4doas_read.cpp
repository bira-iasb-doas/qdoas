...
//  Name of module    :  FRM4DOAS_READ.C
//
...
//  MODULE DESCRIPTION
//
//  Functions to read spectra from netCDF files (format used for FRM4DOAS project).
//
//  ----------------------------------------------------------------------------
//
//  FUNCTIONS
...
//  HISTORY
//
// 18/12/2017 : creation date (Caroline FAYT)
//  ----------------------------------------------------------------------------
...
namespace
 {
  ...
  struct location
   {
     float instr_longitude,instr_latitude,instr_altitude;
   };
  ...
  struct metadata
   {
    vector<float> sza, saa, vea, vaa,                                           // solar zenith angle, solar azimuth angle, viewing elevation angle, viewing azimuth angle
                  tat, tmt, lon, lat, tint;                                      // total acquisition time, total measurement time, longitude, latitude, exposure time
    vector<int>   mt;                                                           // measurement type
    vector<short> dt,dts,dte;                                                   // datetime, datetime_start, datetime_end
    vector<short> sci;                                                          // scan index
   };
  ...
  struct measurements
   {
    vector<float> spe,err,wve;                                                  // spectrum, instrumental errors, wavelength calibration
    vector<short> qf;                                                           // quality flags, scan index
   };
 }
...
static metadata current_metadata;
static measurements current_measurements;
...
// static location read_location(const NetCDFGroup& location_group, size_t num_rec) {
//
//   location result;
//
//   {
...
//   using std::ref;
//   std::array<std::tuple<const string, vector<float>&, size_t>, 3> metadatavars {
//     make_tuple("altitude", ref(&result.instr_altitude), 1),
//     make_tuple("latitude", ref(&result.instr_latitude), 1),
//     make_tuple("longitude", ref(&result.instr_longitude), 1)};
//
//   for (auto& var : metadatavars) {
...
//      FILE *fp;
//      fp=fopen("toto.dat","a+t");
//      fprintf(fp,"var %s %d %d\n",name.c_str(),num_rec,elem_size);
//      fclose(fp);
//     }
//
//
//     target.resize(num_rec * elem_size);
//     const size_t start[] = {0, 0, 0, 0};
//     const size_t count[] = {1, num_rec, 1, elem_size};
//
//     location_group.getVar(name, start, count, target.data() );
//
//   }
//
//   {
//    FILE *fp;
//    fp=fopen("toto.dat","a+t");
//    fprintf(fp,"---> %g %g %g\n",result.instr_altitude,result.instr_longitude,result.instr_altitude);
//    fclose(fp);
//   }
//
//   return result;
...
static metadata read_metadata(size_t num_rec)
 {
  // Declarations

  metadata result;                                                              // metadata
  const size_t start[] = {0};                                                   // there is no reason not to start from 0
  const size_t count[] = {num_rec};                                             // for these variables, only one dimensions
  const size_t datetime_start[] = {0,0};                                        // there is no reason not to start from 0
  const size_t datetime_count[] = {num_rec,7};                                  // for datetime variables, two dimensions

  // Get metadata variables

  NetCDFGroup metadata_group = current_file.getGroup(root_name+"/metadata");

  metadata_group.getVar("azimuth_viewing_angle",start,count,1,(float)-1.,result.vaa);
  metadata_group.getVar("elevation_viewing_angle",start,count,1,(float)-1.,result.vea);
  ...
  metadata_group.getVar("latitude",start,count,1,(float)-1.,result.lat);
  metadata_group.getVar("longitude",start,count,1,(float)-1.,result.lon);
  ...
  metadata_group.getVar("measurement_type",start,count,1,(int)-1,result.mt);
  ...
  // Scan index is retrieved from measurements
...
int frm4doas_set(ENGINE_CONTEXT *pEngineContext) {
  ...
  try {
    current_file = NetCDFFile(pEngineContext->fileInfo.fileName,NC_NOWRITE); // pEngineContext->fileInfo.fileName);
    root_name = current_file.getName();

    NetCDFGroup root_group = current_file.getGroup(root_name);

    pEngineContext->recordInfo.n_alongtrack=pEngineContext->recordNumber=root_group.dimLen("num_rec");
    pEngineContext->recordInfo.n_crosstrack=1;
    det_size=root_group.dimLen("det_size");

    // Read metadata

    current_metadata = read_metadata(pEngineContext->recordNumber);

    // Read keydata (for analysis)

    current_measurements.spe.resize(det_size);
    current_measurements.err.resize(det_size);
    current_measurements.wve.resize(det_size);
    current_measurements.qf.resize(det_size);

    for (int i=0;i<pEngineContext->recordNumber;i++)
     {
      current_measurements.spe[i]=(double)0.;
      current_measurements.wve[i]=(double)0.;
      current_measurements.err[i]=(double)1.;

      current_measurements.qf[i]=(short)1;
     }
...
// static void get_metadata(RECORD_INFO *pRecord, const metadata& geo, int record) {
...
int frm4doas_read(ENGINE_CONTEXT *pEngineContext,int recordNo,int dateFlag,int localDay)
 {
  ...
  NetCDFGroup measurements_group;
  ...
  if ((recordNo<=0) || (recordNo>pEngineContext->recordNumber))
  ...
  else
   {
    ...
    pRecordInfo->altitude=-1.; // current_metadata.sza[recordNo-1];
...
//                                       band_names[pEngineContext->project.instrumental.frm4doas.spectralBand]);
//   } catch(std::runtime_error& e) {
//     rc = ERROR_SetLast(__func__, ERROR_TYPE_FATAL, ERROR_ID_NETCDF, e.what());
//   }
//
...
// static vector<NetCDFFile> get_reference_orbits(const std::string& input_file, enum frm4doasSpectralBand band, const std::string& reference_dir = "") {
...
//     // for the way frm4doas files are stored at BIRA:
//
...
//     NetCDFGroup geo_group(orbit.getGroup(current_band + "_RADIANCE/STANDARD_MODE/metadata"));
...
//
//   // A radiance reference for frm4doas is created, either
//   //
...
//       // is not empty)
//     (strlen(pEngineContext->project.instrumental.frm4doas.reference_orbit_dir)
...
//                                                                     pEngineContext->project.instrumental.frm4doas.spectralBand,
//                                                                     pEngineContext->project.instrumental.frm4doas.reference_orbit_dir), cache);
...
//     }
//   } catch (std::runtime_error e) {
//     return ERROR_SetLast(__func__, ERROR_TYPE_WARNING, ERROR_ID_frm4doas_REF, pEngineContext->fileInfo.fileName, 0, e.what()); // TODO: more specific error message
//   }
//   for (size_t row = 0; row!= size_groundpixel; ++row) {
//     if (!pEngineContext->project.instrumental.use_row[row]) continue;
...
// }
//
// int frm4doas_get_reference(const char *filename, int pixel,
...
//     }
//   } catch(std::runtime_error& e) {
//     rc = ERROR_SetLast(__func__, ERROR_TYPE_WARNING, ERROR_ID_frm4doas_REF, filename, pixel, e.what());
//   }
//
//   return rc;
// }
