...
unsigned char *usampAnalysisMethod[USAMP_METHOD_MAX]={"Optical density","Intensity fitting"};
USAMP  USAMP_buffers;
FFT    usampFFT;
...
// QDOAS ???   // Declarations
// QDOAS ???
// QDOAS ???   unsigned char fileName1[MAX_ITEM_TEXT_LEN+1],fileName2[MAX_ITEM_TEXT_LEN+1];
...
// QDOAS ???
// QDOAS ???   pSlit->slitType=(unsigned char)SendMessage(GetDlgItem(hwndUsamp,TOOL_USAMP_SLIT_TYPE),CB_GETCURSEL,0,0);
// QDOAS ???   USAMP_buffer.analysisMethod=(unsigned char)SendMessage(GetDlgItem(hwndUsamp,TOOL_USAMP_ANALYSIS_METHOD),CB_GETCURSEL,0,0);
// QDOAS ???
...
// QDOAS ???
// QDOAS ???   USAMP_buffer.noComment=(unsigned char)(IsDlgButtonChecked(hwndUsamp,TOOL_USAMP_NOCOMMENT)==BST_CHECKED)?(unsigned char)1:(unsigned char)0;
// QDOAS ???
...
// QDOAS ???             (unsigned char *) USAMP_options.path,
// QDOAS ???             (unsigned char *) USAMP_options.path2,
// QDOAS ???             (unsigned char *) USAMP_options.calibrationFile,
...
// QDOAS ???             (unsigned char *) USAMP_options.slitConv.slitFile,
// QDOAS ???             (unsigned char *)&USAMP_options.kuruczFile,
