...
RC UsampBuildCrossSections(double *phase1,                                      // OUTPUT : phase 1 calculation
                           ...
                           double *gomeLembda,                                  // GOME calibration
                           double *gomeLembda2,                                 // shifted GOME calibration
                           ...
                           double *kuruczLembda,                                // Kurucz high resolution wavelength scale
                           ...
                           INT     analysisMethod)                              // analysis method
 {
  ...
  for (indexMin=0;indexMin<nGome;indexMin++)
   if (gomeLembda2[indexMin]>(double)1.e-3)
    break;

  for (indexMax=nGome-1;indexMax>=0;indexMax--)
   if (gomeLembda2[indexMax]>(double)1.e-3)
    break;
  ...
  else
   {
    ...
    if (phase1!=NULL)
     ...
     for (i=1,phase1[0]=(double)0.;(i<nGome) && !rc;i++)
      {
       if (gomeLembda2[i]<=(double)1.e-3)
        phase1[i]=(double)0.;
       else if (!(rc=SPLINE_Vector(kuruczLembda,kuruczConvolved,kuruczConvolvedDeriv2,nKurucz,&gomeLembda2[i],&over,1,SPLINE_CUBIC,"UsampBuildCrossSections ")) &&
                !(rc=SPLINE_Vector(gomeLembda,kuruczInterpolated,kuruczInterpolatedDeriv2,nGome,&gomeLembda2[i],&under,1,SPLINE_CUBIC,"UsampBuildCrossSections ")))
    ...
    if ((phase2!=NULL) &&
       !(rc=SPLINE_Vector(kuruczLembda,kuruczConvolved,kuruczConvolvedDeriv2,nKurucz,gomeLembda2,resample,nGome,SPLINE_CUBIC,"UsampBuildCrossSections ")) && // calculate solar spectrum at GOME positions
       !(rc=SPLINE_Deriv2(&gomeLembda2[indexMin],&resample[indexMin],&d2res[indexMin],(indexMax-indexMin+1),"USAMP_Build (resample 2) ")))
     ...
     for (i=0;(i<nGome) && !rc;i++)
      {
       ...
       if (gomeLembda2[i]<(double)1.e-3)
        phase2[i]=(double)0.;
       else if (!(rc=SPLINE_Vector(&gomeLembda2[indexMin],&resample[indexMin],&d2res[indexMin],(indexMax-indexMin+1),&gomeLembda[i],&under,1,SPLINE_CUBIC,"UsampBuildCrossSections ")))
...
RC UsampBuild(double *phase1,                                                   // OUTPUT : phase 1 calculation
              ...
              double *gomeLembda,                                               // GOME calibration
              INT     nGome,                                                    // size of GOME calibration
              double *kuruczLembda,                                             // Kurucz calibration
              ...
              INT     analysisMethod)                                           // analysis method
 {
  ...
  double *kuruczConvolved,*kuruczConvolvedDeriv2,*gomeLembda2,slitParam,
         *resample,*d2res;
  INT     slitType;
  INDEX   i;
  RC      rc;
  ...
  kuruczConvolvedDeriv2=gomeLembda2=resample=d2res=NULL;
  ...
  if (((kuruczConvolved=MEMORY_AllocDVector("UsampBuild ","kuruczConvolved",0,nKurucz))==NULL) ||
      ...
      ((gomeLembda2=MEMORY_AllocDVector("UsampBuild ","gomeLembda2",0,nGome))==NULL) ||
  ...
  else
   {
    ...
    xsnew.lembda=kuruczLembda;
    xsnew.vector=kuruczConvolved;
    xsnew.deriv2=kuruczConvolvedDeriv2;
    xsnew.NDET=nKurucz;

    xshr.lembda=kuruczLembda;
    xshr.vector=kuruczSpectrum;
    xshr.deriv2=kuruczDeriv2;
    xshr.NDET=nKurucz;

    for (i=1,gomeLembda2[0]=gomeLembda[0]-fraction;i<nGome;i++)
     gomeLembda2[i]=gomeLembda[i]-fraction;
//     gomeLembda2[i]=(double)(1.-fraction)*gomeLembda[i-1]+fraction*gomeLembda[i];

    // Kurucz spectrum convolution with a gaussian

    if (!(rc=XSCONV_LoadSlitFunction(&slitFunction,pSlit,&slitParam,&slitType)) &&
//        !(rc=XSCONV_TypeStandardFFT(&usampFFT,slitType,pSlit->slitParam,pSlit->slitParam2,kuruczLembda,kuruczConvolved,nKurucz)) &&
        ...
        !(rc=SPLINE_Deriv2(kuruczLembda,kuruczConvolved,kuruczConvolvedDeriv2,nKurucz,"UsampBuild (kuruczConvolved) ")) &&
        !(rc=SPLINE_Vector(kuruczLembda,kuruczConvolved,kuruczConvolvedDeriv2,nKurucz,gomeLembda,resample,nGome,SPLINE_CUBIC,"UsampBuild (kuruczConvolved) ")) && // calculate solar spectrum at GOME positions
        !(rc=SPLINE_Deriv2(gomeLembda,resample,d2res,nGome,"UsampBuild (resample 1) ")))

     rc=UsampBuildCrossSections(phase1,                     // OUTPUT : phase 1 calculation
                                phase2,                     // OUTPUT : phase 2 calculation
                                gomeLembda,                 // GOME calibration
                                gomeLembda2,                // shifted GOME calibration
                                ...
                                kuruczLembda,               // Kurucz high resolution wavelength scale
  ...
  if (gomeLembda2!=NULL)
   MEMORY_ReleaseDVector("UsampBuild ","gomeLembda2",gomeLembda2,0);
...
RC USAMP_BuildFromAnalysis(INT analysisFlag,INT gomeFlag)
 {
  ...
  INT slitType;
  double slitParam2,*lembda,*lembda2,slitParam[MAX_KURUCZ_FWHM_PARAM],lembda0,x0;
  ...
  if (((lembda=(double *)MEMORY_AllocDVector("USAMP_BuildFromAnalysis ","lembda",0,NDET-1))==NULL) ||
      ((lembda2=(double *)MEMORY_AllocDVector("USAMP_BuildFromAnalysis ","lembda2",0,NDET-1))==NULL))
   ...
   for (indexFeno=0;(indexFeno<NFeno)&&!rc;indexFeno++)
    {
     ...
     if (!pTabFeno->hidden && (pTabFeno->useUsamp) &&
      ...
      {
       // Build lembda for second phase

       memcpy(lembda,pTabFeno->LembdaK,sizeof(double)*NDET);

       if ((analysisFlag==2) && (pUsamp->method==PRJCT_USAMP_FIXED))
        {
        for (j=0,lembda0=pTabFeno->LembdaK[(SvdPDeb+SvdPFin)/2];j<NDET;j++)
         {
          x0=lembda[j]-lembda0;
          lembda[j]-=(pTabFeno->Shift+(pTabFeno->Stretch+pTabFeno->Stretch2*x0)*x0);
         }
        }

       memcpy(lembda2,ANALYSE_zeros,sizeof(double)*NDET);

       indexPixMin=0;
       indexPixMax=NDET;

       if (pUsamp->method==PRJCT_USAMP_FIXED)
        for (i=indexPixMin+1,lembda2[indexPixMin]=lembda[indexPixMin]-pUsamp->phase;i<indexPixMax;i++)
         lembda2[i]=lembda[i]-pUsamp->phase; //  (double)(1.-pUsamp->phase)*lembda[i-1]+pUsamp->phase*lembda[i];
       else
        {
         memcpy(lembda2,ANALYSE_shift,sizeof(double)*pTabFeno->NDET);
         ...
         if (pAnalysisOptions->units==PRJCT_ANLYS_UNITS_PIXELS)
          {
           ...
           rc=SPLINE_Vector(ANALYSE_splineX,lembda,ANALYSE_splineX2,pTabFeno->NDET,&ANALYSE_shift[indexPixMin],
                              &lembda2[indexPixMin],(indexPixMax-indexPixMin+1),pAnalysisOptions->interpol,"USAMP_BuildFromAnalysis ");
          }
        }
       ...
       xsKurucz.lembda=&USAMP_buffers.hrSolar.lembda[USAMP_buffers.lembdaRange[0][indexFeno]];
       xsKurucz.NDET=USAMP_buffers.lembdaRange[1][indexFeno];
       ...
       // Convolution with user-defined slit function
       ...
       else
        {
         if (!pTabFeno->useKurucz ||                                         // don't apply calibration
             !pKuruczOptions->fwhmFit)                                       // apply calibration but don't fit the slit function
          {
           ...
           if ((ANALYSIS_slit.matrix!=NULL) && ANALYSIS_slit.nl && ANALYSIS_slit.nc)
            {
             xsSlit.lembda=ANALYSIS_slit.matrix[ANALYSIS_slit.basec]+ANALYSIS_slit.basel;
           ...
           if (!(rc=XSCONV_TypeStandard(&xsKurucz,0,xsKurucz.NDET,&USAMP_buffers.hrSolar,&xsSlit,&USAMP_buffers.hrSolar,NULL,
            ...
            rc=SPLINE_Deriv2(xsKurucz.lembda,xsKurucz.vector,xsKurucz.deriv2,xsKurucz.NDET,"USAMP_BuildFromAnalysis (2) ");
          }
         ...
         // Convolution with slit function resulting from Kurucz
          ...
          {
           xsSlit.lembda=pTabFeno->LembdaK;
           ...
           if (slitType==SLIT_TYPE_VOIGT)
             ...
             for (i=0;(i<xsKurucz.NDET) && !rc;i++)
              {
               for (indexParam=0;indexParam<MAX_KURUCZ_FWHM_PARAM;indexParam++)
                ...
                else if ((rc=SPLINE_Vector(pTabFeno->LembdaK,pTabFeno->fwhmVector[indexParam],pTabFeno->fwhmDeriv2[indexParam],pTabFeno->NDET,
                                              &xsKurucz.lembda[i],&slitParam[indexParam],1,pAnalysisOptions->interpol,"USAMP_BuildFromAnalysis "))!=ERROR_ID_NO)
                 break;
                }
           ...
           else
            {
             ...
             if ((xsSlit.deriv2=(double *)MEMORY_AllocDVector("USAMP_BuildFromAnalysis ","deriv2",0,pTabFeno->NDET-1))==NULL)
              rc=ERROR_ID_ALLOC;
             else if (!(rc=SPLINE_Deriv2(xsSlit.lembda,xsSlit.vector,xsSlit.deriv2,xsSlit.NDET,"USAMP_BuildFromAnalysis (Slit) ")))
         ...
         if (!rc &&
            !(rc=SPLINE_Deriv2(xsKurucz.lembda,xsKurucz.vector,xsKurucz.deriv2,xsKurucz.NDET,"USAMP_BuildFromAnalysis (1) ")))
          {
           ...
           indexPixMin=USAMP_buffers.lembdaRange[2][indexFeno]=FNPixel(lembda,xsKurucz.lembda[0],pTabFeno->NDET,PIXEL_AFTER);
           indexPixMax=FNPixel(lembda,xsKurucz.lembda[xsKurucz.NDET-1],pTabFeno->NDET,PIXEL_BEFORE);
           USAMP_buffers.lembdaRange[3][indexFeno]=indexPixMax-USAMP_buffers.lembdaRange[2][indexFeno]+1;

           if (!(rc=SPLINE_Vector(xsKurucz.lembda,xsKurucz.vector,xsKurucz.deriv2,xsKurucz.NDET,
                                    &lembda[indexPixMin],&USAMP_buffers.kuruczInterpolated[indexFeno][indexPixMin],(indexPixMax-indexPixMin+1),
                                     pAnalysisOptions->interpol,"USAMP_BuildFromAnalysis ")))

            rc=SPLINE_Deriv2(&lembda[indexPixMin],&USAMP_buffers.kuruczInterpolated[indexFeno][indexPixMin],
         ...
         if (!rc)
          ...
          ((pTabFeno->indexUsamp1!=ITEM_NONE)?&pTabFeno->TabCross[pTabFeno->indexUsamp1].vector[USAMP_buffers.lembdaRange[2][indexFeno]]:NULL,
           (pTabFeno->indexUsamp2!=ITEM_NONE)?&pTabFeno->TabCross[pTabFeno->indexUsamp2].vector[USAMP_buffers.lembdaRange[2][indexFeno]]:NULL,
            &lembda[USAMP_buffers.lembdaRange[2][indexFeno]],
            &lembda2[USAMP_buffers.lembdaRange[2][indexFeno]],
            &USAMP_buffers.kuruczInterpolated[indexFeno][USAMP_buffers.lembdaRange[2][indexFeno]],
            &USAMP_buffers.kuruczInterpolated2[indexFeno][USAMP_buffers.lembdaRange[2][indexFeno]],
             USAMP_buffers.lembdaRange[3][indexFeno],
             xsKurucz.lembda,
             xsKurucz.vector,
             xsKurucz.deriv2,
             xsKurucz.NDET,
  ...
  if (lembda!=NULL)
   MEMORY_ReleaseDVector("USAMP_BuildFromAnalysis ","lembda",lembda,0);
  if (lembda2!=NULL)
   MEMORY_ReleaseDVector("USAMP_BuildFromAnalysis ","lembda2",lembda2,0);
...
//
// INPUT         lembdaMin, lembdaMax : range of wavelengths for the kurucz spectrum
...
RC USAMP_GlobalAlloc(double lembdaMin,double lembdaMax,INT size)
 {
  ...
  if ((strlen(pKuruczOptions->file)==strlen(pUsamp->kuruczFile)) &&
       ...
       KURUCZ_buffers.hrSolar.NDET)
   {
    if ((rc=XSCONV_Alloc(&USAMP_buffers.hrSolar,KURUCZ_buffers.hrSolar.NDET,1))!=ERROR_ID_NO)
     ...
     {
      memcpy(USAMP_buffers.hrSolar.lembda,KURUCZ_buffers.hrSolar.lembda,sizeof(double)*KURUCZ_buffers.hrSolar.NDET);
  ...
  else
   {
    ...
    if (!(rc=XSCONV_LoadCrossSectionFile(&USAMP_buffers.hrSolar,kuruczFile,lembdaMin-7.,lembdaMax+7.,(double)0.,CONVOLUTION_CONVERSION_NONE)))
  ...
  else if (((USAMP_buffers.lembdaRange[0]=(INT *)MEMORY_AllocBuffer("USAMP_GlobalAlloc ","lembdaRange[0]",NFeno,sizeof(INT),0,MEMORY_TYPE_INT))==NULL) ||
           ((USAMP_buffers.lembdaRange[1]=(INT *)MEMORY_AllocBuffer("USAMP_GlobalAlloc ","lembdaRange[1]",NFeno,sizeof(INT),0,MEMORY_TYPE_INT))==NULL) ||
           ((USAMP_buffers.lembdaRange[2]=(INT *)MEMORY_AllocBuffer("USAMP_GlobalAlloc ","lembdaRange[2]",NFeno,sizeof(INT),0,MEMORY_TYPE_INT))==NULL) ||
           ((USAMP_buffers.lembdaRange[3]=(INT *)MEMORY_AllocBuffer("USAMP_GlobalAlloc ","lembdaRange[3]",NFeno,sizeof(INT),0,MEMORY_TYPE_INT))==NULL) ||
    ...
    for (indexFeno=0;(indexFeno<NFeno) && !rc;indexFeno++)
     {
      ...
      USAMP_buffers.lembdaRange[0][indexFeno]=
      USAMP_buffers.lembdaRange[1][indexFeno]=
      USAMP_buffers.lembdaRange[2][indexFeno]=
      USAMP_buffers.lembdaRange[3][indexFeno]=ITEM_NONE;
      ...
      if (!pTabFeno->hidden && pTabFeno->useUsamp &&
       ...
       USAMP_buffers.lembdaRange[0][indexFeno]=USAMP_buffers.lembdaRange[1][indexFeno]=ITEM_NONE;
     }
   }
  ...
  for (indexFeno=0;(indexFeno<NFeno) && !rc;indexFeno++)
   {
    ...
    if (!pTabFeno->hidden && pTabFeno->useUsamp
     ...
     {
      USAMP_buffers.lembdaRange[0][indexFeno]=FNPixel(USAMP_buffers.hrSolar.lembda,(double)pTabFeno->svd.LFenetre[0][0]-7.,USAMP_buffers.hrSolar.NDET,PIXEL_AFTER);
      endPixel=FNPixel(USAMP_buffers.hrSolar.lembda,(double)pTabFeno->svd.LFenetre[pTabFeno->svd.Z-1][1]+7.,USAMP_buffers.hrSolar.NDET,PIXEL_BEFORE);

      lembdaSize=USAMP_buffers.lembdaRange[1][indexFeno]=endPixel-USAMP_buffers.lembdaRange[0][indexFeno]+1;

      if (((USAMP_buffers.kuruczConvoluted[indexFeno]=(double *)MEMORY_AllocDVector("USAMP_LocalAlloc ","kuruczConvoluted",0,lembdaSize-1))==NULL) ||
          ((USAMP_buffers.kuruczConvoluted2[indexFeno]=(double *)MEMORY_AllocDVector("USAMP_LocalAlloc ","kuruczConvoluted2",0,lembdaSize-1))==NULL))
...
void USAMP_GlobalFree(void)
 {
  ...
  if (USAMP_buffers.lembdaRange[0]!=NULL)
   MEMORY_ReleaseBuffer("USAMP_GlobalFree ","lembdaRange[0]",USAMP_buffers.lembdaRange[0]);
  if (USAMP_buffers.lembdaRange[1]!=NULL)
   MEMORY_ReleaseBuffer("USAMP_GlobalFree ","lembdaRange[1]",USAMP_buffers.lembdaRange[1]);
  if (USAMP_buffers.lembdaRange[2]!=NULL)
   MEMORY_ReleaseBuffer("USAMP_GlobalFree ","lembdaRange[2]",USAMP_buffers.lembdaRange[2]);
  if (USAMP_buffers.lembdaRange[3]!=NULL)
   MEMORY_ReleaseBuffer("USAMP_GlobalFree ","lembdaRange[3]",USAMP_buffers.lembdaRange[3]);
