...
RC OPUS_Set(ENGINE_CONTEXT *pEngineContext,FILE *specFp)
 {
  ...
  DoasU32  MagicNumber;                                  // OPUS Signature
  ...
  pEngineContext->opus.NumberPoints = (DoasU32) 0;            // Number of Data Points
  ...
  if (specFp==NULL)
  ...
  else if (!fread(&MagicNumber,sizeof(DoasU32),1,specFp))
  ...
  else
   {
    ...
    for (i=0;(i<Header.CurBlk) && !rc;i++)
     {
      fseek(specFp,(DoasI32)Header.DirPtr+sizeof(DIR_BLOCK)*i,SEEK_SET);           // Set Pointer to Dir Block
      ...
      if (!fread(&DirBlock,sizeof(DIR_BLOCK),1,specFp))                         // Directory Block Read out
      ...
      else
       {
        ...
        switch ( (DoasI32) DirBlock.Type & 131063L )
         {
       // --------------------------------------------------------------
       ...
       // --------------------------------------------------------------
          ...
          if ((!pInstr->user && (((DoasI32)DirBlock.Type&131063L)==1047L)) ||
              ((pInstr->user==1) && (((DoasI32)DirBlock.Type&131063L)==5143L)))
           {
            do
            {
             found=1;
             ...
             if (!strcmp(ParBlock.Name,"NPT"))
              ...
              fseek(specFp,(DoasI32)ParBlock.Rs<<1,SEEK_CUR);
            }
            while (1);
            ...
            if (pEngineContext->opus.NumberPoints>(DoasU32)NDET)
...
RC OPUS_Read(ENGINE_CONTEXT *pEngineContext,int recordNo,int dateFlag,int localDay,FILE *specFp)
 {
  ...
  DoasU32  MagicNumber,NumberPoints;                                              // OPUS Signature
  ...
  if ((recordNo>0) && (recordNo<=pEngineContext->recordNumber))
   {
    ...
    if (fp==NULL)
    ...
    else
     {
      ...
      if (!fread(&MagicNumber,sizeof(DoasU32),1,fp))
      ...
      else
       {
        ...
        for (i=0;(i<Header.CurBlk) && !rc;i++)
         {
          fseek(fp,(DoasI32)Header.DirPtr+sizeof(DIR_BLOCK)*i,SEEK_SET);               // Set Pointer to Dir Block
          ...
          if (!fread(&DirBlock,sizeof(DIR_BLOCK),1,fp))                         // Directory Block Read out
           ...
           // --------------------------------------------------------------
              ...
              if ((!pInstr->user && (((DoasI32)DirBlock.Type&131063L)==1031L)) ||
                  ((pInstr->user==1) && (((DoasI32)DirBlock.Type&131063L)==5127L)))
               {
                 ...
                 ((pInstr->user==1) && (((DoasI32)DirBlock.Type&131063L)==5143L)))
               {
                do
                {
                 ...
                 if (!strcmp(ParBlock.Name,"NPT"))
                  ...
                  fseek(fp,(DoasI32)ParBlock.Rs<<1,SEEK_CUR);
                }
                while (1);
