...
RC MATRIX_Allocate(MATRIX_OBJECT *pMatrix,INT nl,INT nc,INT basel,INT basec,INT allocateDeriv2,unsigned char *callingFunction)
 {
  ...
  unsigned char functionName[MAX_FCT_LEN+1];                                            // the name of the calling function
...
void MATRIX_Free(MATRIX_OBJECT *pMatrix,unsigned char *callingFunction)
 {
  // Declaration

  unsigned char functionName[MAX_FCT_LEN+1];
...
RC MATRIX_Load(unsigned char *fileName,MATRIX_OBJECT *pMatrix,
               INT basel,INT basec,INT nl,INT nc,double xmin,double xmax,
               INT allocateDeriv2,INT reverseFlag,unsigned char *callingFunction)
 {
  // Declarations

  unsigned char   *oldColumn,*nextColumn,                                               // get lines from the input file
  ...
  else if (((oldColumn=(unsigned char*)MEMORY_AllocBuffer("MATRIX_Load","oldColumn",MAX_ITEM_TEXT_LEN+1,sizeof(unsigned char),0,MEMORY_TYPE_STRING))==NULL) ||
           ((nextColumn=(unsigned char*)MEMORY_AllocBuffer("MATRIX_Load","nextColumn",MAX_ITEM_TEXT_LEN+1,sizeof(unsigned char),0,MEMORY_TYPE_STRING))==NULL))
