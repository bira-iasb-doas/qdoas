...
unsigned char       TREE_editFlag=0;                     // flag set when editing a tree item label
...
// QDOAS ???   unsigned char     *pDataList;     // list of objects of the same type as the node to delete
...
// QDOAS ???         ((pDataList=(unsigned char *)TREE_itemType[(dataType=pTree->dataType)].dataList)!=NULL) &&   // list of objects of the same type as the node to delete
...
// QDOAS ???
// QDOAS ???   unsigned char     *dataList,        // list of objects child nodes belong to
...
// QDOAS ???         ((dataList=(unsigned char *)TREE_itemType[dataType].dataList)!=NULL) &&                                                                                // list of objects child nodes belong to
// QDOAS ???         ((dataSize=TREE_itemType[dataType].dataSize)!=0) &&                                                                                            // size in bytes needed for the data description of child nodes
// QDOAS ???         ((dataTmp=(unsigned char *)MEMORY_AllocBuffer("TREE_SortChildNodes ","dataTmp",dataSize,1,0,MEMORY_TYPE_STRING))!=NULL) &&                                                  // temporary buffer to receive the description of data when swapping child nodes
...
// QDOAS ???
// QDOAS ???   unsigned char          *pDataList;       // list of objects of the same type as the input 'dataType'
// QDOAS ???   SZ_LEN          dataNameLen;     // length of the input 'dataName'
// QDOAS ???
// QDOAS ???   // Initializations
// QDOAS ???
// QDOAS ???   pTreeItemType=&TREE_itemType[dataType];
// QDOAS ???   pDataList=(unsigned char *)pTreeItemType->dataList;
...
// QDOAS ??? // -----------------------------------------------------------------------------
// QDOAS ???
// QDOAS ??? INDEX TREE_GetIndexByDataIndex(INDEX dataIndex,unsigned char dataType,INDEX entryPoint)
// QDOAS ???  {
// QDOAS ???   // Declarations
// QDOAS ???
// QDOAS ???   INDEX indexItem,                 // browse child nodes
...
// QDOAS ???                          unsigned char *textItem,    // the text of the new item to insert
...
// QDOAS ???                          unsigned char  newItem,     // flag set for loading properties dialog box after insertion
// QDOAS ???                          unsigned char  folderFlag,  // flag set for a folder (user-defined or directory)
// QDOAS ???                          unsigned char  hidden)      // flag set if item is hidden
// QDOAS ???  {
// QDOAS ???   // Declarations
// QDOAS ???
// QDOAS ???   unsigned char fileName[MAX_ITEM_TEXT_LEN+1];       // if the new item is a file/folder, complete file name
// QDOAS ???   TREE_ITEM *pTree,*pTreeParent;
// QDOAS ???   TREE_ITEM_TYPE *pTreeItemSpec;
// QDOAS ???   INDEX indexItem,indexData;
// QDOAS ???   unsigned char folderFlagParent;                    // folder specification for the parent node
// QDOAS ???   unsigned char *ptr,*ptr2;                          // pointer to parts of the file name
// QDOAS ???
// QDOAS ???   // Initializations
// QDOAS ???
...
// QDOAS ???
// QDOAS ???   folderFlagParent=(unsigned char)(((parentItem!=ITEM_NONE) && (TREE_itemList[parentItem].dataType==TREE_ITEM_TYPE_FILE_CHILDREN))?
...
// QDOAS ???    }
// QDOAS ???
// QDOAS ???   if ((((indexData=TREE_GetIndexByDataName(textItem,(unsigned char)dataType,parentItem))==ITEM_NONE) ||  // there is no item in the tree with the same name
...
// QDOAS ???     if ((folderFlag==RAW_FOLDER_TYPE_PATH) || (folderFlag==RAW_FOLDER_TYPE_PATHSUB))
// QDOAS ???      {
// QDOAS ???       unsigned char tmpBuffer[MAX_ITEM_TEXT_LEN+1];
...
// QDOAS ???        {
// QDOAS ???         unsigned char *pList=(unsigned char *)pTreeItemSpec->dataList;
...
// QDOAS ???
// QDOAS ???         strncpy((unsigned char *)&pList[pTree->dataIndex*pTreeItemSpec->dataSize],
...
// QDOAS ???   unsigned char textItem[MAX_ITEM_NAME_LEN+1],                                                     // new text after editing label
...
// QDOAS ???
// QDOAS ???     if (TREE_GetIndexByDataName(textItem,(unsigned char)pItem->dataType,pItem->parentItem)!=ITEM_NONE)
...
// QDOAS ???
// QDOAS ???         if ((dataList=(unsigned char *)pTreeType->dataList)!=NULL)
// QDOAS ???          strncpy((unsigned char *)&dataList[pItem->dataIndex*pTreeType->dataSize],
...
// QDOAS ???   // Declarations
// QDOAS ???
// QDOAS ???   unsigned char textItem[MAX_ITEM_DESC_LEN+1];     // default string for new item
...
// QDOAS ???             (unsigned char)!folderFlag,(unsigned char)folderFlag,0))==ITEM_NONE))
// QDOAS ???
...
// QDOAS ???   TREE_ITEM *pTree;           // pointer to the selected item
// QDOAS ???   TREE_ITEM_TYPE *pTreeType;  // pointer to the type description of the selected item
// QDOAS ???   unsigned char path[MAX_PATH_LEN+1];
// QDOAS ???   FILE *fp;
// QDOAS ???
// QDOAS ???   if ((indexItem=TREE_GetSelectedItem(hwndTree))!=ITEM_NONE)
// QDOAS ???    {
...
// QDOAS ???   unsigned char          *pData2Copy,           // pointer to the data list to copy
...
// QDOAS ???         ((pData2Copy=(unsigned char *)pTreeType->dataList)!=NULL) &&
// QDOAS ???         ((pData2Paste=(unsigned char *)pTreeType->data2Paste)!=NULL))
// QDOAS ???      {
...
// QDOAS ???              ((pData2Copy=(unsigned char *)pTreeChildType->dataList)!=NULL) &&
// QDOAS ???              ((pData2Paste=(unsigned char *)pTreeChildType->data2Paste)!=NULL))
// QDOAS ???      {
...
// QDOAS ???   unsigned char     *dataList,                       // objects list child nodes to paste belong to
...
// QDOAS ???         ((dataList=(unsigned char *)TREE_itemType[dataType].dataList)!=NULL) &&
// QDOAS ???         ((pData2Paste=(unsigned char *)TREE_itemType[dataType].data2Paste)!=NULL) &&
...
// QDOAS ???          {
// QDOAS ???           if (((indexNewItem=TREE_GetIndexByDataName(&pData2Paste[i*dataSize],(unsigned char)dataType,indexItem))==ITEM_NONE) &&
...
// QDOAS ???   unsigned char         menuItemText[MAX_ITEM_TEXT_LEN+1];  // text of the current menu item
