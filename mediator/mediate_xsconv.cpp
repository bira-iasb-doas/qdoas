...
RC mediateConvolutionSave(QWidget *parent,void *engineContext)
 {
  // Declarations
  ...
  if ((fp=fopen(fileName,"rt"))!=NULL)
   {
    UCHAR message[MAX_STR_LEN+1];
    sprintf(message,"File '%s' already exist.  Overwrite ?",fileName);

    QMessageBox::StandardButton yesno=QMessageBox::question(parent,"Convolution tool",message,
  					     QMessageBox::Yes|QMessageBox::No,QMessageBox::Yes);

    rc=(yesno==QMessageBox::Yes)?0:ERROR_SetLast("mediateConvolutionSave",ERROR_TYPE_WARNING,ERROR_ID_FILE_OVERWRITE);
   }
      ...
      (!deconvFlag || !(rc=XSCONV_LoadSlitFunction(&XSCONV_slitDFunction,pSlitDConv,&slitParam2,&slitType2))))))
   {
    ...
    // Load high resolution Kurucz file in convolution with I0 correction method
     ...
     // ----------------------------------------------------------------------
      ...
      if (!rc && !(rc=mediateConvolutionSave(parent,pEngineContext)) && dispConv)
       {
