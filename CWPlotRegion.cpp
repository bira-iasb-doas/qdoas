  ...
  m_plotPage(NULL)
{
  ...
  RefCountPtr<const CPlotDataSet> c2(new CPlotDataSet(x2,y2,4,"guff","lambda","Intensity"));
...
void CWPlotRegion::addPage(RefCountPtr<const CPlotPageData> page)
{
  // the page must not already exist
  std::map< int,RefCountPtr<const CPlotPageData> >::iterator it = m_pageMap.find(page->pageNumber());
  if (it == m_pageMap.end())
    m_pageMap.insert(std::map< int,RefCountPtr<const CPlotPageData> >::value_type(page->pageNumber(), page));
...
{
  std::map< int,RefCountPtr<const CPlotPageData> >::iterator it = m_pageMap.find(pageNumber);
  if (it != m_pageMap.end()) {
